...
class PathFinder:
    ...
    def walkable(self):
        ...
        walkable = [
            add(hns, mul((x, y), self.node_size))
...
def setup_collisions(space, enemy_types, player_type):
    ...
    for etype in enemy_types:
        handler = space.add_collision_handler(player_type, etype)
    ...
    def enemy_enemy_solve(arbiter, space, data):
        ...
        eshape.body.position = (
            eshape1.body.position + (normal * (eshape.radius * 2)) + perp_move
        )
        return True

    for etype1, etype2 in it.combinations(enemy_types, 2):
        handler = space.add_collision_handler(etype1, etype2)
