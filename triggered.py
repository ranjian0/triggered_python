...
# -- creating entities
class Player(sprite, kinematicbody):
    health: int = 100
    move_speed: float = 0
    direction: Vector2 = Vector2(0, 0)
    look_at: Vector2 = Vector2(0, 0)
    ...
    def update(self, dt):
        move_vec = direction * move_speed * dt
        self.move_and_collide(move_vec)

        angle = angle(self.look_at)
        self.rotate(angle)

    def event(self, ev):
        if ev == EventType.KEY_PRESS:
            symbol, mod = args
            if symbol == key.W:
                direction.y = 1
...
class MainMenu(Scene):
    ...
    title: Label = Label("Triggered")

    new_game: Button = Button("New Game")
    ...
    def start(self):
        ...
        self.new_game.connect("clicked", self.play)
        self.quit.connect("clicked", self.quit)

    def play(self):
        Application.instance.process(GameScene())

    def quit(self):
        Application.instance.quit()

class PauseScene(Scene): pass
class LevelFailed(Scene): pass
class LevelFailed(Scene): pass
...
app = Application((800, 600), "Triggered")
app.process(Game())
app.run()
